document.addEventListener('DOMContentLoaded', async () => {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            const response = await fetch(`/common/student-subjects?${params.toString()}`, {
                method: "GET",
            ...
            if (Array.isArray(data?.data?.subjects) && data?.data?.subjects?.length > 0) {
                let tableHTML = `
                ...
                data?.data?.subjects.forEach(item => {
                        tableHTML += `
                            <tr>
